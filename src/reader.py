...
class Vocabulary_s(object):
    def __init__(self, vocab_file, num_entities, num_types):
        self.vocab = self.load_vocab(vocab_file)
        self.inv_vocab = {v: k for k, v in self.vocab.items()}
        self.num_entities = num_entities
        self.num_types = num_types
        assert len(self.vocab) == self.num_entities + self.num_types
    def load_vocab(self, vocab_file):
        """
        Load a vocabulary file into a dictionary.
        """
...
def read_schema_examples(input_file):
    """
    Read a schema file into a list of NaryExample.
    """
    examples = []
    ent2type = open(input_file, encoding='utf-8')
    for num, line in enumerate(ent2type):
    ...
    ent2type.close()
    examples = list(set(examples))  # 352759 -> 145531
...
def cl_batching(input_file, vocabulary, num_relations, num_entities, batchsize):
    ent2type = open(input_file, encoding='utf-8')
    type_dict = dict()
    for num, line in enumerate(ent2type):
        item = line.strip().split('\t')
        ...
        type_ls = types.split('.')
        type_ls = type_ls[::-1]
        nearest_type = type_ls[0]
        if nearest_type in type_dict:
            if ent not in type_dict[nearest_type]:
                type_dict[nearest_type].append(vocabulary.convert_tokens_to_ids([ent])[0])
        else:
            type_dict[nearest_type] = []
            type_dict[nearest_type].append(vocabulary.convert_tokens_to_ids([ent])[0])

        # nearest_type2 = type_ls[1]
        # if nearest_type2 in type_dict:
        #     if ent not in type_dict[nearest_type2]:
        #         type_dict[nearest_type2].append(vocabulary.convert_tokens_to_ids([ent])[0])
        # else:
        #     type_dict[nearest_type2] = []
        #     type_dict[nearest_type2].append(vocabulary.convert_tokens_to_ids([ent])[0])

    ent2type.close()
    cl_batch = []
    for i in type_dict:
        num_ent = len(type_dict[i])
        if num_ent > batchsize:
            ent_ls = type_dict[i]
            random.shuffle(ent_ls)
            sub_batch = num_ent // batchsize
            for j in range(sub_batch):
                schema_batch = np.array(ent_ls[(batchsize * j):(batchsize * (j + 1))]).astype("int64")
                globl_batch = schema_batch + 2 + num_relations
                curr_batch = [globl_batch, schema_batch]
                cl_batch.append(curr_batch)
