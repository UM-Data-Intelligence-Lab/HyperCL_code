...
parser.add_argument("--vocab_size", type=int, default=47688) #jf17k:29148
parser.add_argument("--vocab_schema_size", type=int, default=47688) #jf17k:29393 wikipeople:38221 wd50k:53475
parser.add_argument("--vocab_file", type=str, default="./data/wd50k/vocab.txt") #"./data/jf17k/vocab.txt"
parser.add_argument("--vocab_schema_file", type=str, default="./data/wd50k/vocab_schema.txt") #"./data/jf17k/vocab_schema.txt"
parser.add_argument("--ent2types_file", type=str, default="./data/wd50k/entity2types_ttv.txt") #"./data/jf17k/entity2types_ttv.txt"
...
parser.add_argument("--num_relations", type=int, default=500) #501
parser.add_argument("--num_types", type=int, default=500) # jf17k:748 wikipeople:3396 wd50k:6320
...
parser.add_argument("--cl_batch_size", type=int, default=1024) # 1024
parser.add_argument("--lr", type=float, default=5e-4) # 5e-4
parser.add_argument("--cl_lr", type=float, default=5e-4) # 5e-4
...
def main(limit=1e9):
    ...
    vocabulary_s = Vocabulary_s(
        vocab_file=args.vocab_schema_file,
        num_entities=args.num_entities,
    ...
    schema_examples = read_schema_examples(args.ent2types_file)
    ...
    schema_graph = build_schema_graph(vocabulary_s, schema_examples, device)
    cl_batch = cl_batching(args.ent2types_file, vocabulary_s, args.num_relations, args.num_entities, args.cl_batch_size)

    
    if len(devices) > 1:
        model = torch.nn.DataParallel(Transformer(graph, args.vocab_size, schema_graph, args.vocab_schema_size, args.num_relations,
                                                  args.num_entities, args.local_layers, args.global_layers, args.hidden_dim,
                                                  args.local_heads, args.global_heads, args.use_global, args.local_dropout,
                                                  args.global_dropout, args.decoder_activation, args.global_activation,
                                                  args.use_edge, args.remove_mask, args.use_node, device), device_ids=devices)
        model.to(device)
    else:
        model = Transformer(graph, args.vocab_size, schema_graph, args.vocab_schema_size, args.num_relations, args.num_entities,
                            args.local_layers, args.global_layers, args.hidden_dim, args.local_heads, args.global_heads,
                            args.use_global, args.local_dropout, args.global_dropout, args.decoder_activation, args.global_activation,
                            args.use_edge, args.remove_mask, args.use_node, device).to(device)
    ...
    optimizer_cl = torch.optim.AdamW(model.parameters(), weight_decay=args.weight_decay)
    scheduler_cl = OneCycleLR(optimizer_cl, max_lr=args.cl_lr, total_steps=max_train_steps,
                            pct_start=args.warmup_proportion, anneal_strategy="linear", cycle_momentum=False)
    ...
    for epoch in range(limit):
        ...
        for n_batch_cl in cl_batch:
            model.train()
            cl_loss = model('cl', n_batch_cl[0], n_batch_cl[1])
        ...
        logger.info(f"cl_epoch_loss: {cl_epoch_loss:.3f}")
        logger.info(f"epoch: {epoch}\tepoch_loss: {epoch_loss:.3f}\tlr: {scheduler.get_last_lr()[0]:.6f}\ttrain time: {hours:02d}:{minutes:02d}:{seconds:02d}")
...
def predict(model, test_loader, all_features, vocabulary, device):
    ...
    with torch.no_grad():
        for item in tqdm(test_loader):
            ...
            output = model('test', input_ids, input_mask, mask_position, mask_output, edge_labels)
